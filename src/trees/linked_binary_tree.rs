...
pub struct BinTree<T: std::cmp::PartialEq> {
    root: Box<Node<T>>, // Needs Option for emptry trees
    size: usize,
}
/** The Binary Tree's API includes the following functions:
 *
*/
impl<T> BinTree<T>
where
    ...
    T: std::cmp::PartialEq
{
    ...
    //    }
    //    Ok(node.parent.as_ref().expect("Y U NO ROOT NO HAS PAREN?"))
    //        //node.parent
    //        //.as_ref()
