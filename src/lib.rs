...
# Lists
These structures/modules represent a basic exploration of foundational, sequence-based structures. The concepts illustrated here can be used for more advanced data structures. I originally wrote 9+ lists, but have only decided to include 3 in this library due to overlapping utility. The project's scope includes eventually adding an index-based list (like `Vec<T>`), but for now linked-based structures will have to do. All lists still exist in the repo for historical reference purposes. The [array experiment](https://github.com/p5chmitz/dsa-rust/blob/main/src/lists/array_list.rs) gets a special mention as an example of how to use arrays in Rust, due to the necessity of compile-time sizing and initialization.

- [Singly-linked list](crate::lists::singly_linked_list): A singly-linked structure over owned values written entirely in safe code that illustrates `Box`. This implementation contains operations for simple stack and queue structures as well.

- [Doubly-linked list](crate::lists::doubly_linked_list): A doubly-linked list implementation over owned values written with ample use of raw pointers. This list includes a cursor to create a positional list implementation that provides list splitting/splicing out of the box. These additions allow the list to support in-place sorting, if you choose to implement it.
