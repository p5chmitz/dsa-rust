...
- [Unsafe doubly-linked list](https://github.com/p5chmitz/dsa-rust/blob/main/src/lists/doubly_linked_list_2.rs): My first attempt at a naive and horribly unsafe doubly-linked list with raw, mutable (and null-able) pointers everywhere; This implementation is balanced out with just enough Miri testing to not immediately cause concern among friends and loved ones; Represents a key-value list that maintains a sorted invariant over its elements; Removes entries by name
- [Unsafe generic doubly-linked list](https://github.com/p5chmitz/dsa-rust/blob/main/src/lists/generic_doubly_linked_list.rs): The result of actually reading Learn Rust With Entirely Too Many Linked Lists; This is a basic, unsorted, doubly-linked list that contains a cursor and split/splice functions for a positional implementation that supports in-place sorting with a merge sort algorithm (if you really wanna write a SortedList on top of it)
